...
file_path = r"special_pairs5m.txt"
...
def fetch_initial_data():
    ...
    # Initialize an empty list to collect timestamps across all tickers
    all_timestamps = []

    for ticker in tickers:
        # Define the time range: from now to the last 500 minutes to ensure we get enough data points
        end_time = datetime.now(eastern)  # Get the current time in US/Eastern (handles EDT/EST)
        start_time = end_time - timedelta(minutes=700)
        ...
        if response.status_code == 200:
            ...
            if 'results' in data and len(data['results']) > 0:
                ...
                # Collect timestamps for this ticker
                all_timestamps.append(df.index)
                
                # Store the fetched data and the latest closing price
                ticker_data[ticker] = {
                    'close_prices': df,
                    'current_price': df['Close'].iloc[-1]  # Store only the latest price as a scalar
                }
    ...
    # Reindex all tickers' dataframes to have the same timestamps
    if all_timestamps:
        # Get the union of all unique timestamps across all tickers
        all_timestamps = pd.Index(sorted(set().union(*all_timestamps)))
        
        for ticker in ticker_data:
            # Reindex to ensure all dataframes have the same timestamps and forward fill missing values
            ticker_data[ticker]['close_prices'] = ticker_data[ticker]['close_prices'].reindex(all_timestamps).ffill()
            print(ticker,ticker_data[ticker])
    return ticker_data
...
def place_trades():
    ...
    for first_stock, second_stock in pairs:
        ...
        if z_score is not None:  # Ensure z_score is calculable
            z_score_value = z_score.item()  # Convert the Series to a scalar value
            
            # Extract scalar values for the current prices
            ticker1_price = ticker_data[first_stock]['current_price']
            ticker2_price = ticker_data[second_stock]['current_price']
            
            z_scores.append((first_stock, second_stock, z_score_value, ticker1_price, ticker2_price))
            ...
            if pair_key in current_trades:
                ...
                current_trades[pair_key]['z_score'] = z_score_value
...
def order_management():
    ...
    for key, trade_info in current_trades.items():
        ...
        if z_score is not None:
            z_score_value = z_score.item()  # Convert the z-score Series to a scalar value
            ...
            if -0.5 < z_score_value < 0.5:  # Now compare the scalar z-score value
                if direction1 == "BUY":
                    buy_stock = ticker2
                    sell_stock = ticker1
                ...
                print(f'Exiting trade for {ticker1} and {ticker2} due to z_score of {z_score_value}')
